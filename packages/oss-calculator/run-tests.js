...
function assertArrayEquals(actual, expected, message) {
  ...
  for (let i = 0; i < actual.length; i++) {
    if (actual[i] !== expected[i]) {
      throw new Error(message || `Expected element ${i} to be ${expected[i]} but got ${actual[i]}`);
    }
  }
}
...
test('VAT Rates: EU_VAT_RATES should contain 27 Member States', () => {
  const rateFile = fs.readFileSync(path.join(__dirname, 'src/vat-rates.ts'), 'utf-8');
...
test('VAT Rates: Germany reduced rate is 7%', () => {
  ...
  assert(
    rateFile.includes('rate: 7') || rateFile.includes('rate: 7,'),
    'Germany reduced rate is 7%',
  );
});

// ===== TAX ENGINE TESTS =====
test('Tax Engine: Module exports Transaction interface', () => {
  const engineFile = fs.readFileSync(path.join(__dirname, 'src/tax-engine.ts'), 'utf-8');
...
test('Threshold Monitor: Module exports ThresholdMonitor class', () => {
  const thresholdFile = fs.readFileSync(path.join(__dirname, 'src/threshold-monitor.ts'), 'utf-8');

  assert(thresholdFile.includes('export class ThresholdMonitor'), 'ThresholdMonitor class exists');
  assert(
...
test('Threshold Monitor: Tracks threshold status and monthly breakdown', () => {
  ...
  assert(thresholdFile.includes('enum ThresholdStatus'), 'ThresholdStatus enum exists');
  assert(thresholdFile.includes('BELOW_THRESHOLD'), 'BELOW_THRESHOLD status exists');
  assert(thresholdFile.includes('AT_OR_ABOVE_THRESHOLD'), 'AT_OR_ABOVE_THRESHOLD status exists');
  assert(thresholdFile.includes('getMonthlyBreakdown'), 'Monthly breakdown tracking exists');
});

// ===== ECB RATES TESTS =====
test('ECB Rates: Module exports ECBRateProvider class', () => {
  const ecbFile = fs.readFileSync(path.join(__dirname, 'src/ecb-rates.ts'), 'utf-8');

  assert(ecbFile.includes('export class ECBRateProvider'), 'ECBRateProvider class exists');
...
test('ECB Rates: Module exports CurrencyConverter class', () => {
  ...
  assert(ecbFile.includes('export class CurrencyConverter'), 'CurrencyConverter class exists');
...
test('ECB Rates: ECB_DECIMAL_PLACES defines currency rounding conventions', () => {
  ...
  assert(ecbFile.includes('export const ECB_DECIMAL_PLACES'), 'ECB_DECIMAL_PLACES constant exists');
...
test('Quarterly Aggregator: Module exports QuarterlyAggregator class', () => {
  const aggFile = fs.readFileSync(path.join(__dirname, 'src/quarterly-aggregator.ts'), 'utf-8');

  assert(aggFile.includes('export class QuarterlyAggregator'), 'QuarterlyAggregator class exists');
...
test('Quarterly Aggregator: Generates 4-section NAP Bulgaria return structure', () => {
  ...
  assert(aggFile.includes("'2A'"), 'Section 2A exists (Services from supplier)');
  assert(aggFile.includes("'2B'"), 'Section 2B exists (Goods from supplier)');
  assert(aggFile.includes("'2C'"), 'Section 2C exists (Services from other MS)');
  assert(aggFile.includes("'2D'"), 'Section 2D exists (Goods from other MS)');
});
...
test('Errors: Module exports all three scenario error classes', () => {
  const errFile = fs.readFileSync(path.join(__dirname, 'src/errors.ts'), 'utf-8');
...
test('Errors: Error classes include error codes and context', () => {
  ...
  assert(errFile.includes('code: string'), 'Error base class has code property');
  assert(
    errFile.includes('context?: Record<string, unknown>'),
    'Error base class has context property',
  );
  assert(errFile.includes("'MISSING_COUNTRY_CODE'"), 'Missing country code error has code');
  assert(errFile.includes("'RATE_MISMATCH'"), 'Rate mismatch error has code');
  assert(errFile.includes("'CURRENCY_ROUNDING_DIVERGENCE'"), 'Currency rounding error has code');
});

// ===== INDEX EXPORTS TESTS =====
test('Index: Module re-exports all public APIs', () => {
  const indexFile = fs.readFileSync(path.join(__dirname, 'src/index.ts'), 'utf-8');
...
test('Test Coverage: vat-rates.test.ts exists with comprehensive tests', () => {
  const testFile = fs.readFileSync(path.join(__dirname, 'src/vat-rates.test.ts'), 'utf-8');
...
test('Test Coverage: tax-engine.test.ts exists with comprehensive tests', () => {
  const testFile = fs.readFileSync(path.join(__dirname, 'src/tax-engine.test.ts'), 'utf-8');
...
test('Test Coverage: ecb-rates.test.ts exists', () => {
  assert(fs.existsSync(path.join(__dirname, 'src/ecb-rates.test.ts')), 'ecb-rates tests exist');
});

test('Test Coverage: errors.test.ts exists', () => {
  assert(fs.existsSync(path.join(__dirname, 'src/errors.test.ts')), 'errors tests exist');
});
...
test('Build: vitest.config.ts exists', () => {
  assert(fs.existsSync(path.join(__dirname, 'vitest.config.ts')), 'vitest configuration exists');
});
...
test('Academic Quality: All modules have JSDoc comments', () => {
  ...
  for (const file of files) {
    const content = fs.readFileSync(path.join(__dirname, `src/${file}`), 'utf-8');
